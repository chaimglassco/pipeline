...
ADMIN-only backup calls use `POST /api/library-state` with `create-backup` or `restore-backup`; backup list/detail use `GET /api/library-state?backups=1` and `GET /api/library-state?backupId=...`. Restore requires `expectedRevision` and creates a before-restore backup first.
