...
`GET /api/library-state` returns `{ state, revision, initialized, recordVersions, recordManifest, catalogCompleteness, updatedAt, updatedBy }`. `recordManifest.documents` contains the ID, slug, record version, lifecycle (`active`, `deleted`, or `archived`), visibility, and publication status for every protected document. `catalogCompleteness` identifies the read scope and carries same-snapshot expected/returned document and category counts. A malformed record or count mismatch returns structured HTTP `503` with code `LIBRARY_CATALOG_INCOMPLETE`; the API never emits a shorter successful response. ADMIN recovery reads additionally include optional `deletionAudit.documents`, keyed by document ID, with `source`, `deletedAt`, `reason`, direct `actor`, and system-operation `initiatedBy` metadata. `state` remains `{ version: 1, documents, categories }`; normal reads return active records, `recovery=1` opts into tombstones, and `archive=1` opts into protected archive records. `recordVersions.documents` and `recordVersions.categories` map selected stable record IDs to monotonic integer versions. `initialized` is false while revision is zero and becomes true only after the one-time `catalog.initialize` migration.

`PATCH /api/library-state` uses a flat mutation body such as `{ operation: "document.update", documentId, expectedVersion, document }`. Supported operation types are `catalog.initialize`, `document.create`, `document.update`, `document.delete`, `document.restore`, `document.archive` (plus the legacy `document.purge` alias), `document.restoreArchived`, `documents.restoreSystemDeleted`, `documents.reorder`, `category.create`, `category.update`, `category.delete`, `category.restore`, `category.archive`, and `categories.reorder`. Permanent deletion from normal Recovery is implemented as movement into the protected archive; physical table deletion is blocked by a database trigger. `documents.restoreSystemDeleted` is ADMIN-only and requires a non-empty unique `documentIds` list plus `expectedRevision`; all requested records must be currently tombstoned with a latest `system_migration` deletion event or the transaction changes nothing. Updates and lifecycle transitions require `expectedVersion`; reorder and initialize require `expectedRevision`. A stale expectation returns HTTP `409`, `conflict: true`, and the current full response payload. The older nested `{ operation: { type, ...fields } }` envelope is accepted only for backward compatibility.

Before initialization, every operation except ADMIN `catalog.initialize` returns `409`; each mutation also enforces this guard inside its atomic SQL statement. Document update always preserves `id`, `slug`, and tombstone state. USER may edit content and normal metadata such as type, tags, and legacy body, but USER updates preserve `hidden` and publication `status`; ADMIN may change those fields. USER-created documents are forced to `hidden: false`, `status: "published"`, and cannot provide a tombstone. Direct writes must satisfy the stable Library document/category envelope and enum/list validation used by the reader; structured element and rich-text payloads are validated without removing additional forward-compatible properties.

Normal catalog/document GET handling never calls integrity repair, restoration, backup merge, or any record mutation. Trusted-version selection used by explicit maintenance is deterministic by record version, creation time, and version ID. A missing record or lifecycle mismatch creates an unacknowledged integrity incident and does not recreate or transition the record.

ADMIN-only backup calls use `POST /api/library-state` with `create-backup` or `restore-backup`; backup list/detail use `GET /api/library-state?backups=1` and `GET /api/library-state?backupId=...`. Restore requires `expectedRevision` and creates a before-restore backup first. Restore merges backup records without tombstoning backup-absent documents/categories, preserves newer active documents, and refuses to convert an active record into a tombstone.
