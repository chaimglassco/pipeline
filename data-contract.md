...
`GET /api/library-state` returns `{ state, revision, initialized, recordVersions, updatedAt, updatedBy }`. ADMIN responses additionally include optional `deletionAudit.documents`, keyed by document ID, with `source`, `deletedAt`, `reason`, direct `actor`, and system-operation `initiatedBy` metadata. `state` remains `{ version: 1, documents, categories }`; `recordVersions.documents` and `recordVersions.categories` map stable record IDs to monotonic integer versions. `initialized` is false while revision is zero and becomes true only after the one-time `catalog.initialize` migration. Tombstoned records remain in the response so authorized recovery tools can display them.

`PATCH /api/library-state` uses a flat mutation body such as `{ operation: "document.update", documentId, expectedVersion, document }`. Supported operation types are `catalog.initialize`, `document.create`, `document.update`, `document.delete`, `document.restore`, `document.purge`, `documents.restoreSystemDeleted`, `documents.reorder`, `category.create`, `category.update`, `category.delete`, `category.restore`, and `categories.reorder`. `document.purge` is ADMIN-only, requires `documentId` plus `expectedVersion`, and physically removes only an existing tombstone while retaining a metadata-only actor audit event. Backup restore excludes every ID with a `document.purge` audit, so purged content cannot be recreated from an older backup. `documents.restoreSystemDeleted` is ADMIN-only and requires a non-empty unique `documentIds` list plus `expectedRevision`; all requested records must be currently tombstoned with a latest `system_migration` deletion event or the transaction changes nothing. Updates/deletes/restores/purges require `expectedVersion`; reorder and initialize require `expectedRevision`. A stale expectation returns HTTP `409`, `conflict: true`, and the current full response payload. The older nested `{ operation: { type, ...fields } }` envelope is accepted only for backward compatibility.
