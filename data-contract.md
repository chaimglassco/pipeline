...
`PATCH /api/library-state` uses a flat mutation body such as `{ operation: "document.update", documentId, expectedVersion, document }`. Supported operation types are `catalog.initialize`, `document.create`, `document.update`, `document.delete`, `document.restore`, `documents.restoreSystemDeleted`, `documents.reorder`, `category.create`, `category.update`, `category.delete`, `category.restore`, and `categories.reorder`. `documents.restoreSystemDeleted` is ADMIN-only and requires a non-empty unique `documentIds` list plus `expectedRevision`; all requested records must be currently tombstoned with a latest `system_migration` deletion event or the transaction changes nothing. Updates/deletes/restores require `expectedVersion`; reorder and initialize require `expectedRevision`. A stale expectation returns HTTP `409`, `conflict: true`, and the current full response payload. The older nested `{ operation: { type, ...fields } }` envelope is accepted only for backward compatibility.
