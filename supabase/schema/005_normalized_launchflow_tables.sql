...
create policy "products_insert_admins" on public.products for insert to authenticated with check (public.is_workspace_member(workspace_id, array['owner', 'admin', 'user']) and created_by = (select auth.uid()));
drop policy if exists "products_update_admins" on public.products;
create policy "products_update_admins" on public.products for update to authenticated using (public.is_workspace_member(workspace_id, array['owner', 'admin', 'user'])) with check (public.is_workspace_member(workspace_id, array['owner', 'admin', 'user']));
...
create policy "stage_field_templates_insert_admins" on public.stage_field_templates for insert to authenticated with check (public.is_workspace_member(workspace_id, array['owner', 'admin', 'user']) and created_by = (select auth.uid()));
drop policy if exists "stage_field_templates_update_admins" on public.stage_field_templates;
create policy "stage_field_templates_update_admins" on public.stage_field_templates for update to authenticated using (public.is_workspace_member(workspace_id, array['owner', 'admin', 'user'])) with check (public.is_workspace_member(workspace_id, array['owner', 'admin', 'user']));
...
create policy "product_stage_details_insert_admins" on public.product_stage_details for insert to authenticated with check (exists (select 1 from public.products p where p.id = product_id and public.is_workspace_member(p.workspace_id, array['owner', 'admin', 'user'])));
...
create policy "launch_monitoring_entries_insert_admins" on public.launch_monitoring_entries for insert to authenticated with check (exists (select 1 from public.products p where p.id = product_id and public.is_workspace_member(p.workspace_id, array['owner', 'admin', 'user'])) and created_by = (select auth.uid()));
...
create policy "campaign_prep_settings_insert_admins" on public.campaign_prep_settings for insert to authenticated with check (exists (select 1 from public.products p where p.id = product_id and public.is_workspace_member(p.workspace_id, array['owner', 'admin', 'user'])) and created_by = (select auth.uid()));
...
create policy "vine_review_feedback_insert_admins" on public.vine_review_feedback for insert to authenticated with check (exists (select 1 from public.products p where p.id = product_id and public.is_workspace_member(p.workspace_id, array['owner', 'admin', 'user'])) and created_by = (select auth.uid()));
