...
`documents.restoreSystemDeleted` atomically restores only IDs whose latest deletion audit is `system_migration`, guarded by the expected catalog revision. Conditional record updates return `409` with the latest authoritative state when a version is stale. Backup restore is a merge: backup-absent records and newer active documents remain untouched, and an active record can never become a tombstone from a backup payload. Category renames cascade to matching active documents in the same database statement.
