...
`documents.restoreSystemDeleted` atomically restores only IDs whose latest deletion audit is `system_migration`, guarded by the expected catalog revision. `document.purge` permanently removes only a version-matched tombstone, retains a metadata-only actor audit event, and marks the ID so backup merge cannot recreate it. Conditional record updates return `409` with the latest authoritative state when a version is stale. Backup restore is a merge: backup-absent records and newer active documents remain untouched, an active record can never become a tombstone from a backup payload, and purged document IDs are excluded. Category renames cascade to matching active documents in the same database statement.
